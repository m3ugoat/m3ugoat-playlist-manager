...
- Two escape hatches from the conditional path, both deliberate: `epgSources.updateStatus()` / `poolSources.updateStatus()` for the background refresh (no request user, must not fail on a version race — it only writes fetch bookkeeping), and `channels.updateUnconditional()` for bulk items whose caller supplied no `version` (see the bulk-precondition notes below).
...
- `setSessionToken()` / `clearSessionToken()` in `src/apiClient.ts` dispatch an **`auth-changed`** window event, which `AuthProvider` listens for. This is load-bearing: the provider reads identity once on mount, which happens while the lock screen is still up and `/api/auth/me` 401s, so without the event a fresh login leaves `user` null until a page reload. It mirrors the existing `auth-expired` event in the other direction.
- `LockScreen` asks for a username only when `GET /api/auth/status` reports `multiUser`, and sends a `deviceName` derived from the UA string so the browser is identifiable in the device list.
- `src/components/AccountSettings.tsx` renders the devices list, a **Sign out** button, and (for admins) the account list inside `SettingsPage`. It renders **nothing** when authentication is disabled, since there is no identity to manage.
- **Switching accounts is sign out then sign in** — there is no account picker, because the lock screen is where credentials are entered. `signOut()` clears the token, leaves Settings (so signing back in lands on the dashboard) and dispatches `auth-expired`, which is the event `App.tsx` listens to in order to show `LockScreen`.
...
- The bulk routes (`bulk-update`, `bulk-update-many`, `bulk-replace`, `bulk-delete`, `reorder`) take no **`If-Match`** — one precondition cannot cover many rows with independent versions. They accept an optional **`versions`** map (`{channelId: version}`) instead; `bulk-update-many` takes a per-entry `version`, since its payload is already per-channel. A malformed map is **400**, kept distinct from a conflict.
- Bulk preconditions **skip and report** rather than refusing the batch: matching items apply, stale ones come back in `conflicts` as `{id, expected, current}` alongside an `applied`/`deleted` count. Failing a whole batch because one row moved would be hostile, and a sync client needs to know which items to re-merge. **Omitting `versions` keeps last-write-wins**, which is what the web UI relies on — it sends none.
- `channels.setOrderIfVersion()` exists for the reorder route specifically: a version-checked single-column write, cheaper than `update()`, which rewrites every column.
...
1. **My Playlists** — `PlaylistEditor.tsx` + `CategoryList.tsx`. Its Export dialog builds links from the playlist's `exportToken` (`/e/<token>`), **not** `shortId` — the numeric URLs return 410 — and offers a "Generate new links" action that rotates the token. Drag-reorder (dnd-kit) for both channels and categories, inline click-to-edit fields, multi-select bulk move/delete/find-replace, a stream health checker (HEAD, falling back to GET, per channel), TVG-ID autocomplete against the EPG pool, and client-side pagination (100 channels/page) per category.
